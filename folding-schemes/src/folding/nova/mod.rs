...
impl<C: CurveGroup> Witness<C> {
    ...
    {
        Ok(RunningInstance {
            cmE: CS::commit(None, params, &E, &C::ScalarField::zero())?,
            u: C::ScalarField::one(),
            cmQ: CS::commit(None, params, &self.q(), &self.rQ)?,
            cmW: CS::commit(None, params, &self.w(), &self.rW)?,
            x,
        })
    }
    ...
    CS2: CommitmentScheme<C2>,
{
    ...
    pub E: DeviceVec<C1::ScalarField>,
    pub T: DeviceVec<C1::ScalarField>,

    pub cf_E: DeviceVec<C2::ScalarField>,
    pub cf_T: DeviceVec<C2::ScalarField>,
    pub cmW: Option<DeviceVec<<C1::Config as MSM<C1>>::R>>,

    pub stream: CudaStream,
    pub stream2: CudaStream,
    pub stream3: CudaStream,
}
    ...
    for<'a> &'a GC2: GroupOpsBounds<'a, C2, GC2>,
{
    ...
    type RunningCommittedInstanceWithWitness =
        (RunningInstance<C1>, Witness<C1>, Vec<C1::ScalarField>);
    ...
    fn preprocess<R: Rng>(
        ...
        let _ = augmented_F_circuit.run(cs.clone())?;
    ...
    fn init(
        ...
        Ok(Self {
            ...
            E: C1::ScalarField::alloc_vec(vp.r1cs.A.n_rows),
            T: C1::ScalarField::alloc_vec(vp.r1cs.A.n_rows),
            cf_E: C2::ScalarField::alloc_vec(vp.cf_r1cs.A.n_rows),
            cf_T: C2::ScalarField::alloc_vec(vp.cf_r1cs.A.n_rows),
            cmW: None,
            stream: CudaStream::create().unwrap(),
            stream2: CudaStream::create().unwrap(),
            stream3: CudaStream::create().unwrap(),
        })
    }
    ...
    fn prove_step(
        ...
        if self.i == C1::ScalarField::zero() {
            ...
            let (cfWa_u_i, cfWa_W_i1, cfWa_U_i1, cfWa_cmT) = {
                let cf_r1cs = &vp.cf_r1cs;
                let cf_cs_params = &pp.cf_cs_params;
                let cf_W_i = &self.cf_W_i;
                let cf_U_i = &self.cf_U_i;
                ...
                let mut cf_u_i = CycleFoldCommittedInstance {
                    cmE: C2::zero(),
                ...
                let cmW = <C2::Config as MSM<C2>>::var_msm_precomputed(
                    Some(&self.stream2),
                    &cf_cs_params.device_generators,
                    &cf_w_i.QW,
                    0,
                    None,
                );
                ...
                let cf_cmT = NIFS::<C2, CS2>::compute_cyclefold_cmT(
                    Some(&self.stream2),
                    cf_cs_params,
                    cf_r1cs,
                    &cf_W_i,
                ...
                #[cfg(test)]
                {
                    cf_r1cs.check_cyclefold_instance_relation(&cf_w_i, &cf_u_i)?;
                }
                (cf_u_i, cf_W_i1, cf_U_i1, cf_cmT)
            };
            end_timer!(timer);

            let timer = start_timer!(|| "Run and fold CycleFold circuits for W");
            let (cfWb_u_i, cfWb_W_i1, cfWb_U_i1, cfWb_cmT) = {
                let cf_r1cs = &vp.cf_r1cs;
                let cf_cs_params = &pp.cf_cs_params;
                let cf_W_i = &cfWa_W_i1;
                let cf_U_i = &cfWa_U_i1;
                let cf_circuit = CycleFoldCircuit::<C1, GC1> {
                    _gc: PhantomData,
                    r_bits: Some(r_bits.clone()),
            ...
            let (cfE_u_i, cfE_W_i1, cfE_U_i1, cfE_cmT) = {
                let cf_r1cs = &vp.cf_r1cs;
                let cf_cs_params = &pp.cf_cs_params;
                let cf_W_i = &cfWb_W_i1;
                let cf_U_i = &cfWb_U_i1;
                let cf_circuit = CycleFoldCircuit::<C1, GC1> {
                    _gc: PhantomData,
                    r_bits: Some(r_bits.clone()),
            ...
            #[cfg(test)]
            {
                vp.cf_r1cs.check_relaxed_cyclefold_instance_relation(
                    &self.cf_W_i,
                    &self.cf_U_i,
                    C2::ScalarField::retrieve_e(&self.cf_E),
                )?;
            }
        }
        ...
        let z_i1 = augmented_F_circuit.run(cs.clone())?;
        end_timer!(timer);
        ...
        let cmQ = <C1::Config as MSM<C1>>::var_msm_precomputed(
            Some(&self.stream),
            &pp.cs_params.device_generators,
            &cs.borrow()
        ...
        let cmQ = <C1::Config as MSM<C1>>::retrieve_msm_result(Some(&self.stream3), &cmQ);
        end_timer!(timer);
        ...
        let timer = start_timer!(|| "Commit to W");
        let cmW = <C1::Config as MSM<C1>>::var_msm_precomputed(
            Some(&self.stream3),
            &pp.cs_params.device_generators,
        ...
        #[cfg(test)]
        {
            ...
            vp.r1cs.check_relaxed_running_instance_relation(
                &self.W_i,
                &self.U_i,
                C1::ScalarField::retrieve_e(&self.E),
            )?;
        }
    ...
    fn instances(
        ...
        let mut u_i = self.u_i.clone();
        u_i.cmW = <C1::Config as MSM<C1>>::retrieve_msm_result(
            Some(&self.stream),
            &self.cmW.as_ref().unwrap(),
        );
        (
            (
                self.U_i.clone(),
    ...
    fn verify(
        ...
        let (U_i, W_i, E) = running_instance;
        let (u_i, w_i) = incoming_instance;
        let (cf_U_i, cf_W_i, cf_E) = cyclefold_instance;
        ...
        vp.r1cs
            .check_relaxed_running_instance_relation(&W_i, &U_i, E)?;

        // check CycleFold RelaxedR1CS satisfiability
        vp.cf_r1cs
            .check_relaxed_cyclefold_instance_relation(&cf_W_i, &cf_U_i, cf_E)?;
