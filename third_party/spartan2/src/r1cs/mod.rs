...
/// CSR sparse matrix (needed to build [`R1CSShape`] from external R1CS).
...
impl<E: Engine> RelaxedR1CSInstance<E> {
  ...
  /// Absorb `comm_W` and `comm_E` into a Fiat–Shamir transcript.
  ///
  /// Required for **standalone** use of [`crate::spartan_relaxed::RelaxedR1CSSpartanProof`],
  /// which does not absorb commitments itself (it assumes an outer NIFS binding).
  pub fn absorb_commitments(&self, transcript: &mut E::TE) {
    transcript.absorb(b"comm_W", &self.comm_W);
    transcript.absorb(b"comm_E", &self.comm_E);
  }
}

impl<E: Engine> RelaxedR1CSWitness<E> {
  /// Build a relaxed witness from assignments `W` and error vector `E`, and
  /// the matching committed instance `(comm_W, comm_E, u, X)`.
  ///
  /// Pads `W` to `S.num_vars` and `E` to `S.num_cons` with zeros.
  pub fn from_assignments(
    ck: &CommitmentKey<E>,
    S: &R1CSShape<E>,
    mut W: Vec<E::Scalar>,
    mut E_vec: Vec<E::Scalar>,
    u: E::Scalar,
    X: Vec<E::Scalar>,
  ) -> Result<(Self, RelaxedR1CSInstance<E>), SpartanError> {
    if X.len() != S.num_io {
      return Err(SpartanError::InvalidInputLength {
        reason: format!(
          "RelaxedR1CSWitness::from_assignments: Expected {} IO, got {}",
          S.num_io,
          X.len()
        ),
      });
    ...
    if W.len() > S.num_vars {
      return Err(SpartanError::InvalidInputLength {
        reason: format!(
          "RelaxedR1CSWitness::from_assignments: W longer than num_vars ({} > {})",
          W.len(),
          S.num_vars
        ),
      });
    }
    if E_vec.len() > S.num_cons {
      return Err(SpartanError::InvalidInputLength {
        reason: format!(
          "RelaxedR1CSWitness::from_assignments: E longer than num_cons ({} > {})",
          E_vec.len(),
          S.num_cons
        ),
      });
    ...
    E_vec.resize(S.num_cons, E::Scalar::ZERO);

    let r_W = PCS::<E>::blind(ck, S.num_vars);
    let r_E = PCS::<E>::blind(ck, S.num_cons);
    let witness = Self {
      W,
      r_W,
      E: E_vec,
    ...
    let (comm_W, comm_E) = witness.commit(ck)?;
...
impl<E: Engine> R1CSShape<E> {
  ...
  pub fn commitment_key(&self) -> (CommitmentKey<E>, VerifierKey<E>) {
    // Hyrax bases must cover both W (num_vars) and E (num_cons) openings.
    E::PCS::setup(
      b"ck",
      self.num_vars.max(self.num_cons),
